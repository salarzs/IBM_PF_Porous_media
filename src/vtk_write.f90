...
!    real, dimension(1:itot, 1:jtot, 1:ktot) :: v_undisturbed
    ...
    write(istepchar,'(i9.9)') istep
    if (type2d.eq.1) then  
      filename = datadir//'2dFlYZ_'//istepchar//'.vtk'
    elseif (type2d.eq.2) then
      filename = datadir//'2dFlXZ_'//istepchar//'.vtk'
    elseif (type2d.eq.3) then
      filename = datadir//'2dFlXY_'//istepchar//'.vtk'
    endif
...
!
    ...
    write(s_buffer, FMT = '(A,I12)', iostat = iostatus) 'SCALARS C11 double', 1
    write(unit = ifich, iostat = iostatus) trim(s_buffer) // newline
    write(unit = ifich, iostat = iostatus) 'LOOKUP_TABLE default' // newline
