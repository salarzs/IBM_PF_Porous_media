...
indent = indent + 4
  ...
  indent = indent + 4
    ...
    indent = indent + 4
      ...
      do i = 1,nflds!fldstart,fldend,nskip !1,nflds
      ...
      indent = indent + 4
        ...
        do ii = 1,nscal
          ...
          indent = indent + 4
            ...
            write(unit=ixdmf,fmt='(A)')trim(buffer)
            indent = indent + 4
              write(buffer,fmt='(A,i7.7,A)') repeat(' ',indent)//scalname(ii),i,'_3d.bin'
              write(unit=ixdmf,fmt='(A)')trim(buffer)
              indent = indent - 4
            write(buffer,fmt='(A)') repeat(' ',indent)//'</DataItem>'
            write(unit=ixdmf,fmt='(A)')trim(buffer)
        ...
        do ii = 1,nscal
          ...
          indent = indent + 4
            ...
            write(unit=ixdmf,fmt='(A)')trim(buffer)
            indent = indent + 4
              write(buffer,fmt='(A,i7.7,A)') repeat(' ',indent)//scalname(ii),fldinit,'_3d.bin'
              write(unit=ixdmf,fmt='(A)')trim(buffer)
              indent = indent - 4
            write(buffer,fmt='(A)') repeat(' ',indent)//'</DataItem>'
            write(unit=ixdmf,fmt='(A)')trim(buffer)
